...
void RatesAnalysis::MaxRatesAnalysis(
  ...
  for (i = 0; i < ProductsForRates.size(); i++) {
    ...
    if (RatesAnalysisData[ProductsForRates[i].SpeciesID]
  ...
  for (i = 0; i < ReactantsForRates.size(); i++) {
    if (RatesAnalysisData[ReactantsForRates[i].SpeciesID]
...
void RatesAnalysis::WriteMaxRatesAnalysis(
  ...
  filename =
      rates_analysis_stream_filename + "RatesAnalysis-Max-Production-Rate.txt";
  ...
  filename =
      rates_analysis_stream_filename + "RatesAnalysis-Max-Consumption-Rate.txt";
  Outfile.open(filename.c_str());
...
void RatesAnalysis::PrepareStreamRatesAnalysis(
  ...
  string filename;
  filename = rates_analysis_stream_filename +
             "RatesAnalysis-Species-Production-Rate-Stream.txt";
  ...
  filename = rates_analysis_stream_filename +
             "RatesAnalysis-Species-Consumption-Rate-Stream.txt";
  Outfile.open(filename.c_str());
