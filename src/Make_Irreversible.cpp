...
)
{
	...
	LocalReactionParameters = RunIntegrator::Process_Reaction_Parameters(Reactions);
	LocalReactantsForReactions = RunIntegrator::Reactants_ForReactionRate(Reactions);
	LocalProductsForReactions = RunIntegrator::Products_ForReactionRate(Reactions,false);
	...
	vector< Species::ThermodynamicData::CalculatedThermodynamics > CalculatedThermo(Number_Species);
	...
	for(i=0;i<50;i++)
	{
		...
		SolverCalculation::Calculate_Rate_Constant(Kf, Kr, Temperature,LocalReactionParameters, CalculatedThermo, LocalSpeciesLossAll, Local_Delta_N);
