...
void SolverCalculation::ODE_RHS_Gas_Liquid_Phase(int *n, double *time_current,
  ...
  // vector< Sync_ID > Sync_IDs, // this vector will need to include a map:
  // liquid species <-> gas species
  //  alternatively I can re-arrange species at the beginning, this might be
  //  better for performance
  size_t number_synchronized_species = 0;
  size_t liquid_species_count = 0;
  size_t gas_species_count = 0; // gas and liquid counts so I know where
  ...
  Synchronize_Gas_Liquid_Model(number_synchronized_species,
                               liquid_species_count, gas_species_count, y,
                               Vliq_div_Vgas, Henry_Constants);
  ...
  SpeciesConcentrationChange =
      SolverCalculation.SpeciesLossRate(Rates, SpeciesLossAll);
...
// notes:
/* This will need to be fed a combined system of two mechanisms, so they are
 * solved in synch there needs to be a transfer section for rates too that links
 * the two.... The system will be huge for big mechanisms...
 */
