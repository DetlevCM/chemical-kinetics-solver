...
void SolverCalculation::Prepare_Jacobian_Matrix(
  ...
  size_t Number_Species =
      Reactions[0].Reactants.size(); // Consider that temperature is included
  ...
  // Column wise order means we first have to check if the species we
  ...
  for (size_t i = 0; i < Number_Species; i++) // column
  {
    ...
    for (size_t j = 0; j < Number_Species; j++) // row
    {
      ...
      for (size_t k = 0; k < Reactions.size(); k++) {
        ...
        // species production
        if (Reactions[k].Reactants[i] != 0 &&
            Reactions[k].Products[j] != 0) // differentiation species is present
        {
          temp.ColumnWiseArrayPosition =
              i * (Number_Species + 1) + j; // consider temp array position
          temp.IsForward = true;
          temp.IsProduction = true;
          temp.ReactionID = k; // gets me the k
          temp.coefficient =
              (Reactions[k].Reactants[i]) *
              (Reactions[k]
                   .Reactants[i]); // correct for A -> 2B as well, is production
                ...
                0) // differentiation species is present
        {
          ...
          temp.IsProduction = true;
          temp.coefficient =
              Reactions[k].Products[i] *
              Reactions[k]
                  .Products[i]; // correct for A -> 2B as well, is production
  ...
  for(i=0;i<(int) JacobianColumnWise.size();i++)
  {
          ...
          if(JacobianColumnWise[i].IsForward) // Forward
          {
                  ...
                  {
                          //temp =
Kf[JacobianColumnWise[i].ReactionID]*JacobianColumnWise[i].coefficient; cout <<
"Kf" << JacobianColumnWise[i].ReactionID + 1 << "*" <<
JacobianColumnWise[i].coefficient << " ";
                  }
...
JacobianColumnWise[i].coefficient << " ";
          ...
          if(!JacobianColumnWise[i].IsForward) // Reverse
          {
                  ...
                  {
                          //temp =
Kr[JacobianColumnWise[i].ReactionID]*JacobianColumnWise[i].coefficient; cout <<
"Kr" << JacobianColumnWise[i].ReactionID + 1 << "*" <<
JacobianColumnWise[i].coefficient << " ";
                  }
                  if(!JacobianColumnWise[i].IsProduction) // species are lost
                  {
...
JacobianColumnWise[i].coefficient << " ";
          ...
          {
                  //temp = temp *
pow(Concentration[JacobianColumnWise[i].Species[j].SpeciesID],JacobianColumnWise[i].Species[j].power);
                  cout << "Spc(" << JacobianColumnWise[i].Species[j].SpeciesID +
1 << ")^" << JacobianColumnWise[i].Species[j].power << " ";
          }
          cout << "\n";
  }
//*/
