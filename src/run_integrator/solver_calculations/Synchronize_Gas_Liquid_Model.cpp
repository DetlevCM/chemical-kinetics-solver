...
void SolverCalculation::Synchronize_Gas_Liquid_Model(
  ...
  /* From my PhD Thesis
   *
   * C_new = (N_current + C_current * V_sample) R * T / (kH * V_gas + V_sample *
   * R * T)
   *
   * This can be rewritten for concentrations
   *
   * C_new = (gas_C_current + C_current * V_liq/V_gas) * R * T / ( kH +
   * V_liq/V_gas * R * T )
   *
  ...
  for (i = 0; i < number_synchronized_species;
       i++) // this only needs to be done for the synchronised species
  {
    new_liquid_concentration[i] =
        ((y[i + liquid_species_count + 1] + y[i] * Vliq_div_Vgas) * R *
         gas_temperature) /
        (Henry_Constants[i] + Vliq_div_Vgas * R * gas_temperature);
    // this calculation then gives the new gas and liquid concentrations
    // pressures, etc. can be calculated if desired/necessary during the output
    // writing phase
    new_gas_concentration[i] =
        y[i + liquid_species_count + 1] -
        (new_liquid_concentration[i] - y[i]) * Vliq_div_Vgas;
  }
