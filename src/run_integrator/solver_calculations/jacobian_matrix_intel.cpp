...
void SolverCalculation::Jacobian_Matrix_Intel(int *n, double *t, double *y,
                                              double *a) {
  ...
  vector<double> JacobeanColumnWise((Number_Species + 1) *
                                    (Number_Species + 1));
  ...
  for (i = 0; i < JacobianMatrix.size(); i++) {
    ...
    if (JacobianMatrix[i].IsForward) // Forward
    {
      ...
      {
        temp =
            -Kf[JacobianMatrix[i].ReactionID] * JacobianMatrix[i].coefficient;
      }
    }
    ...
    if (!JacobianMatrix[i].IsForward) // Reverse
    {
      ...
      {
        temp =
            -Kr[JacobianMatrix[i].ReactionID] * JacobianMatrix[i].coefficient;
      }
    }
    ...
    for (j = 0; j < JacobianMatrix[i].Species.size(); j++) {
      if (JacobianMatrix[i].Species[j].power != 0) // power 0 = *1
      {
        if (JacobianMatrix[i].Species[j].power ==
            1) // power 1 is simple multiplication
        {
          temp = temp * Concentration[JacobianMatrix[i].Species[j].SpeciesID];
        } else {
          temp =
              temp * pow(Concentration[JacobianMatrix[i].Species[j].SpeciesID],
