...
int RunIntegrator::Prepare_Integrator_Settings(Initial_Data InitialParameters,
  ...
  /*
   * To simplify the model selection, all solvers will be accessible using an
   * integer ID. Every solver will be given an ID number starting from 1,
   * followed by three digits to specify the exact settings.
   *
   * So IntelODE will be 1xxx with the first combination 1001.
   * ODEPACK will carry on with 2xxx, the first setting being 2001.
   *
   * This allows up to 999 settings per solver which is excessive while also
   * allowing a technically unlimited expansion of the list of supported solvers
  ...
  {
    /* Because we do not know the size of kd and dpar in advance we need to be
     * clever - or at least act as we are. By defining kd and dpar as vectors we
     * can assign a size "on the fly" as required. We can further pass a pointer
     * to the vector to the solver, thus this is what we do. Rather than arrays,
     * kd and dpar are pointers to vectors of doubles in our case.
     */

    Intel.vector_ipar.resize(
        128); // vectors initialise to zero, as required by the solver
    for (size_t i = 0; i < 128; i++) {
      Intel.vector_ipar[i] =
          0; // should be unnecessary, vector initialises to zero
    }
    ...
    Intel.ep = InitialParameters.Solver_Parameters
                   .rtol; // relative tolerance. The code cannot guarantee the
                          // requested accuracy for ep<1.d-9
    Intel.tr = InitialParameters.Solver_Parameters.atol /
    ...
    if (!Intel.Solver_Type.Use_Stiff_Solver &&
        !Intel.Solver_Type.Use_Analytical_Jacobian) {
      solver_choice = 1001;
      Intel.solver_subsettings = 1; // stiff solver without analytical Jacobian
    } else if (!Intel.Solver_Type.Use_Stiff_Solver &&
               Intel.Solver_Type.Use_Analytical_Jacobian) {
      solver_choice = 1002;
  ...
  } else if (InitialParameters.Solver_Parameters.SolverType ==
             1) // LSODA Settings
  {
    solver_choice = 2002;
    LSODA.JT =
        2; // default, no Jacobian provided, i.e. use internal numerical one
