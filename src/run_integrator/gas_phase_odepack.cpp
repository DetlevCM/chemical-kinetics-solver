...
//*/
  ...
  do {
    ...
    if (Use_Analytical_Jacobian) {
      int JT = 1;
      dlsoda_((void *)&SolverCalculation::ODE_RHS_Gas_Phase, &n,
              solver_calculation.Concentration.data(), &time_current,
              &time_step, &ITOL, &RTOL, &ATOL, &ITASK, &ISTATE, &IOPT, RWORK,
              &LRW, IWORK, &LIW,
              (void *)&SolverCalculation::Jacobian_Matrix_Odepack_LSODA, &JT);
    ...
    if (ISTATE < 0) {
      ...
      // Break means it should leave the do loop which would be fine for an
      // error response as it stops the solver
      break;
    }
    ...
    WriteOutput::StreamConcentrations(
        ...
        Number_Species, time_current, pressure, SpeciesConcentration);
