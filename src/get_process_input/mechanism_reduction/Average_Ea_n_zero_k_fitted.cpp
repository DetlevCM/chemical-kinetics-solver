...
ArrheniusParameters MechanismReduction::Average_Ea_n_zero_k_fitted(
  ...
  // we need the equivalent of an ln(k) vs T plot for a straight line with a
  // gradient now we need an Ea gradient as well as ln(T) gradient
  ...
  double ln_fitted_a;
  // ln_fitted_a = Group_k[20]- fitted_n*log(temperature) -
  // (-fitted_Ea/temperature);
