...
vector<size_t> MechanismReduction::Map_Species_Classes(
  ...
  OrderedSpeciesMapping.resize(
      Number_Species); // Needs enough space for all species
  ...
  for (i = 0; i < Species_Mapping_Source_Vector; i++) {
    ...
    for (j = 0; j < Number_Species; j++) {
      ...
      if (strcmp(Species_Name.c_str(), Comparator.c_str()) == 0) {
        ...
        // OrderedSpeciesMapping[j] = strtod(SpeciesMapping[i][j].c_str(),NULL);
        // // compare class ID to int
      }
    }
  }
  ...
  for (i = 0; i < Number_Species; i++) {
    if (OrderedSpeciesMapping[i] ==
        0) // every unassigned species is its own species class
    {
      // printf("Species not in class: %u \n",i);
      CountUnmappedSpecies = CountUnmappedSpecies + 1;
    }
    // Assuming Classes are numbered consecutively, the largest class is the
    // number of classes
  ...
  NumberOfClasses =
      size_t(CountUnmappedSpecies +
             BiggestClass); // sum of unmapped species and the number of classes

  cout << "The Mapping consists of " << CountUnmappedSpecies
       << " unmapped Species, and " << BiggestClass
       << " dedicated class mappings, resulting in " << NumberOfClasses
       << " Species Classes. \n";

  // Classed species have a number, unclassed species get identified via a
  // negative ID: This will only work if I coded the rest well as position 0
  // drops out
  ...
  for (i = 0; i < Number_Species; i++) {
    // OrderedSpeciesMapping[i] = abs(OrderedSpeciesMapping[i])-1; // correct to
    // use zero based array and turn negative to positive
    OrderedSpeciesMapping_Output[i] =
        (size_t)abs(OrderedSpeciesMapping[i]) -
        1; // correct to use zero based array and turn negative to positive
  }
