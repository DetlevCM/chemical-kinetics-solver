...
MechanismReduction::Process_Reactions_For_Species_Lumping(
  ...
  for (i = 0; i < Number_Species; i++) {
    SpeciesClassesSize[SpeciesClassMapping[i]]++; // count number of species in
                                                  // each class
  }

  // Per reaction:
  cout << Number_Reactions << " reactions and " << Number_Species_Classes
  ...
  for (i = 0; i < Number_Reactions; i++) {
    ...
    for (j = 0; j < Number_Species; j++) {
      ...
      // need to make sure that two lumped species can react - e.g. R(1). +
      // R(2). -> now lumped into R. which would be 2R. ->
      ReactantData[SpeciesID] =
          ReactantData[SpeciesID] + Reactions[i].Reactants[j];
      ProductData[SpeciesID] =
   ...
   * If we group a  sufficient number of species, it stands to reason that there
   * will be reactions which now seem to describe identical reactions without
   * being duplicates on purpose. Hence we should now identify the redundant
   * reactions and group them to an individual reaction where appropriate. Given
   * that we need to process a large amount of data, we should take something of
   * a sieving approach. Take one reaction, compare the rest and filter out any
   * results. Next we pick the next available reaction and filter again and so
   * on. This approach means that the computational load will decrease as we
   * progress with the reaction combination. In these collection steps, we can
   * assemble the Arrhenius parameters as well to then determine the new
  ...
  do {
    ...
    temp_reactions3.push_back(
        temp_reactions2[0]); // retain the reaction we are testing
    // temp_reactions2.erase (temp_reactions2.begin() + i); // remove the
    // position i points to, will be first if we started searching, i.e. 0
    ...
    do {
      ...
      if (temp_reactions2[j].Reactants ==
              temp_reactions3[compare_position].Reactants &&
          temp_reactions2[j].Products ==
              temp_reactions3[compare_position].Products) {
        // printf("true! \n"); Appears to works :)
        temp_reactions3.push_back(
            temp_reactions2[j]); // retain the reaction we are testing
        temp_reactions2.erase(temp_reactions2.begin() +
                              j); // remove the position i points to, will be
                                  // first if we started searching, i.e. 0
        k = k + 1; // record how many we have grouped together, i.e. added to
                   // the first pick
  ...
  /* We have now rearranged our reactions with identical reactions clustered
   * together, identifiable via the mapping stored in the vector
   * "Reaction_Grouping" and the output available in "temp_reactions3". So what
   * can we do now? Just stupidly averaging the parameters could be a start...
   */

  temp_reactions2.clear(); // wipe the array, it should be empty anyway

  /* Our parameters are stored in temp_reactions3[][2][] -> the first 3
   * parameters for every reaction.
   */

  /*
   * Time to fit some new parameters to the lumped reaction. This is the new
   * method: We allow the user to chose whether he wants a fast lumping method
   * on three points or a slower method that considers every step in a +/-20K
   * interval The fast method should really be always adequate due to the nature
   * of the function
   *
   ...
   * Hence, over a small temperature range (e.g. +/- 20K) it can be approximated
  ...
  // always starts at the start of the vector and then removes the entry, so no
  // stepping through i
  ...
  do {
    ...
    vector<double> ReactantData(
        temp_reactions3[i].Reactants); // Reactant Information
    ...
    if (Reaction_Grouping[i] > 1) {
      if (LumpingType == 1) {
        if (FastLumping) {
          NewParameters = n_zero_k_fitted_Fast(temp_reactions3, temperature,
                                               Reaction_Grouping[i]);
        } else {
          NewParameters = n_zero_k_fitted_Slow(temp_reactions3, temperature,
                                               Reaction_Grouping[i]);
        }
      } else if (LumpingType == 2) {
        if (FastLumping) {
          NewParameters = Average_Ea_k_fitted_Fast(temp_reactions3, temperature,
                                                   Reaction_Grouping[i]);
        } else {
          NewParameters = Average_Ea_k_fitted_Slow(temp_reactions3, temperature,
                                                   Reaction_Grouping[i]);
        }
      } else if (LumpingType == 3) {
        NewParameters = Average_Ea_n_zero_k_fitted(temp_reactions3, temperature,
                                                   Reaction_Grouping[i]);
      }
    ...
    temp_reactions3.erase(temp_reactions3.begin(),
                          temp_reactions3.begin() + delete_positions);
  ...
  for (i = 0; i < temp_reactions2.size(); i++) // every reaction
  {
    ...
    for (j = 0; j < temp_reactions2[i].Reactants.size();
         j++) // every species class
    {
      Scale_A = Scale_A * (pow((double)SpeciesClassesSize[j],
