...
ReactionParameter
MechanismReduction::n_zero_k_fitted_Fast(vector<ReactionParameters> &Reactions,
                                         double temperature,
  ...
  // we have 3 k, so we can use the average of the two gradients to get the
  // middle point
  gradient = 0.5 * ((Group_k[1] - Group_k[0]) / (temperature_endpoints) +
