...
void MechanismReduction::mechanism_picking(
  ...
  if (ReductionData.is_open()) {
    ...
    getline(ReductionData,
            line1); // first entry will specify the type of species picking
...
void MechanismReduction::handle_reactions_with_chosen_species(
  ...
  for (i = 0; i < Number_Reactions; i++) {
    ...
    do {
      if (((Reactions[i].Reactants[j] != 0) &&
           ChosenSpecies[j].Choice) // we want a Reactants
          || ((Reactions[i].Products[j] != 0) &&
              ChosenSpecies[j].Choice) // we want a Product
      ) {
        RunLoop = false;
        retain = true; // PickedReactions.push_back(Reactions[i]); // keep the
                       // picked reaction
      }
    ...
    if (strcmp(type.c_str(), "ONLY") == 0) {
      // second loop to filter out the reactions with an undesirable species
      j = 0;
      do {
        if (((Reactions[i].Reactants[j] != 0) &&
             !ChosenSpecies[j].Choice) // we want a Reactants
  ...
  for (i = 0; i < Number_Reactions; i++) {
    for (j = 0; j < Number_Species; j++) {
      if (Reactions[i].Reactants[j] != 0) // there is a species in the reactants
      {
        ...
        if (Reactions[i].Reactants[j] ==
            -Reactions[i].Products[j]) // same species present
        {
          ...
          if (ChosenSpecies[j].SpcClass != 0) {
            for (k = 0; k < Number_Species; k++) {
              if (ChosenSpecies[j].SpcClass == ChosenSpecies[k].SpcClass &&
                  Reactions[i].Products[k] != 0) {
    ...
    // OK, we know how many reactants and products, how many identical classes,
    // how many identical species

    if ((same_spc_class_reac_prod != 0 && same_spc_reac_prod != 0) &&
        (nbr_spc_reac == nbr_spc_prod) &&
        (nbr_spc_reac == (same_spc_class_reac_prod + same_spc_reac_prod))) {
      // cout << " " << nbr_spc_reac << " " << same_spc_class_reac_prod << " "
      // << same_spc_reac_prod << "\n";
  ...
  if (RecombinationReaction.size() > 0) {
    WriteReactions("Species_Picked_Reactions_regular.txt", species,
                   RegularReaction);
  }

  if (RecombinationReaction.size() > 0) {
    WriteReactions("Species_Picked_Reactions_rearrange.txt", species,
  ...
  Reduce_Species_Thermo_Mechanism(RetainOrNot, species,
                                  // Thermodynamics,
                                  PickedReactions);
