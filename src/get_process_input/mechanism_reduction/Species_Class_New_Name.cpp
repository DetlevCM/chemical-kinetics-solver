...
vector<MechanismReduction::ClassNaming>
MechanismReduction::GetSpeciesClassesNames(
    vector<vector<string>> SpeciesMapping) {
  size_t i;
  size_t MaxClass = 0;
  ...
  for (i = 0; i < SpeciesMapping.size(); i++) {
    ...
    size_t CurrentClass = (size_t)strtol(SpeciesMapping[i][1].c_str(), NULL,
  ...
  for (i = 0; i < temp1.size(); i++) {
    temp2[temp1[i].ClassID - 1] =
        temp1[i]; // beware, visible ID is one higher than class ID
