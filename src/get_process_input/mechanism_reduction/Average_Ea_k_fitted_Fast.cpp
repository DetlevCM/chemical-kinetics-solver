...
ArrheniusParameters MechanismReduction::Average_Ea_k_fitted_Fast(
  ...
  gradient_in_Group_k =
      0.5 * ((Group_k[1] - Group_k[0]) / (temperature_endpoints) +
  ...
  difference_in_gradient =
      gradient_in_Group_k - (gradient_in_Ea + gradient_in_lnT);

  // We can calculate our fitted n now:
  fitted_n = exp(difference_in_gradient);

  // cout << fitted_Ea * 1.987192 / 1000 << " " << gradient_in_Group_k << " " <<
  // gradient_in_Ea << " " << difference_in_gradient << " " << fitted_n << "\n";

  // now for the fitted A, using the center point
  double ln_fitted_A, fitted_A;
  ln_fitted_A =
      Group_k[1] - fitted_n * log(temperature) - (-fitted_Ea / (temperature));
