...
void Initial_Data::Handle_PetroOxy(Initial_Data &InitialParameters,
                                   vector<string> Input,
  ...
  for (i = 0; i < Input.size(); i++) {
    ...
    if (Test_If_Word_Found(Input[i], "Sample Size=")) {
      ...
      InitialParameters.PetroOxy.SampleSize =
          stod(line_content[1], NULL) * 1e-6;
    ...
    if (Test_If_Word_Found(Input[i], "Vessel Size=")) {
      ...
      line_content.clear();
      // Calculated volume from estimate is 22.41mL - so let us assume 22.5mL,
      // value is in m^3
    }
    ...
    if (Test_If_Word_Found(Input[i], "Initial Pressure=")) {
      ...
      // kPa to Pa
      InitialParameters.PetroOxy.InitPressure =
          stod(line_content[1], NULL) * 1000;
    ...
    if (Test_If_Word_Found(Input[i], "Gas Solubility=")) {
      ...
      // kPa to Pa
      InitialParameters.PetroOxy.GasSolubility =
          stod(line_content[1], NULL); // * 1000;
    ...
    if (Test_If_Word_Found(Input[i], "Gas Species=")) {
      ...
      for (size_t i = 0; i < species.size(); i++) {
        if (strcmp(line_content[1].c_str(), species[i].Name.c_str()) == 0) {
      ...
      cout << "Gas Species: "
           << species[InitialParameters.PetroOxy.GasSpecies].Name << "\n";
      line_content.clear();
    }
