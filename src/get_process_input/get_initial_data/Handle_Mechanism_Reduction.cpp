...
Initial_Data::mechanism_reduction
Initial_Data::Handle_Mechanism_Reduction(vector<string> Input) {
  mechanism_reduction Mechanism_Reduction_Output;

  // default
  Mechanism_Reduction_Output.ReduceReactions = 0;
  Mechanism_Reduction_Output.LumpingType =
      1; // original method, n = 0, A & Ea fitted
  ...
  for (i = 0; i < Input.size(); i++) {
    ...
    if (Test_If_Word_Found(Input[i], "Lumping Method 2")) {
      Mechanism_Reduction_Output.LumpingType =
          2; // second method, Ea averaged, n and A fitted
    }
    ...
    if (Test_If_Word_Found(Input[i], "ReduceReactions")) {
      ...
      if ((int)line_content.size() >= 2) {
        Mechanism_Reduction_Output.ReduceReactions =
