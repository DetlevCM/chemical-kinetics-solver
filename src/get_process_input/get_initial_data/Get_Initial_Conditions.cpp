...
void Initial_Data::Get_Initial_Conditions(string filename,
  ...
  Default_Solver_Parameters.SolverType =
      1; // 0 = IntelODE, 1 = odepack (default)
  Default_Solver_Parameters.Use_Stiff_Solver =
      true; // old setting from IntelODE
  Default_Solver_Parameters.Use_Analytical_Jacobian =
      false; // faster with odepack
  ...
  InitialParameters.temperature =
      298.15; // standard condition, avoids segfault if user forgets
  ...
  InitialParameters.MechanismReduction.UseFastLumping =
      true; // old method wasn't wrong but inefficient and slow
  ...
  InitialParameters.PetroOxy.VesselSize =
      22.5; // my estimate of PetroOxy volume
  ...
  string line;          // individually read in line from input file
  ...
  if (!Input_Data.is_open()) {
    ...
    exit(1); // exit code with error status
                 ...
                 .is_open()) { // I need to get the solver parameters first
    while (Input_Data.good()) {
      getline(Input_Data, line);

      if (Line_Not_Comment_Or_Empty(
              line)) // only handle line if not a comment and not empty
      {
        if (Test_If_Word_Found(
                line, "<<no integration>>")) // special case of user not wanting
                                             // integration, e.g. for making a
                                             // scheme irreversible only
        {
              ...
              line)) // only handle line if not a comment and not empty
      {
        if (line.find("<Solver Parameters>") != string::npos) {
          ...
          Global_Solver_Type.Use_Analytical_Jacobian =
              InitialParameters.Solver_Parameters.Use_Analytical_Jacobian;
          Global_Solver_Type.Use_Stiff_Solver =
              InitialParameters.Solver_Parameters.Use_Stiff_Solver;
        }
        Input.clear();
      }
    }
  ...
  Input_Data.open(filename.c_str());
  if (Input_Data.is_open()) { // deal with the rest of the user settings:
    while (Input_Data.good()) {
      getline(Input_Data, line);
      if (Line_Not_Comment_Or_Empty(
              line)) // only handle line if not a comment and not empty
      {
        if (line.find("<Initial Conditions>") != string::npos) {
          ...
          Handle_Initial_Conditions(InitialParameters, Input,
                                    Global_Solver_Type);
        }
        Input.clear();
        ...
        if (line.find("<Species>") != string::npos) {
          ...
          InitialParameters.InitialLiquidSpecies =
              Handle_Species(InitialParameters, Input, species);
        }
        Input.clear();
        ...
        if (line.find("<Gas Species>") != string::npos) {
          ...
          InitialParameters.InitialGasSpecies =
              Handle_Species(InitialParameters, Input, species);
        }
        Input.clear();
        ...
        if (line.find("<Mechanism Reduction>") != string::npos) {
          ...
          InitialParameters.MechanismReduction =
              Handle_Mechanism_Reduction(Input);
  ...
  if (InitialParameters.MechanismAnalysis.RatesAnalysisAtTime &&
      !InitialParameters.irrev) {
    InitialParameters.irrev = true;
    cout << "Rates Analysis At Times requires an irreversible scheme - method "
            "set. \n";
  }
  ...
  if (InitialParameters.MechanismReduction.ReduceReactions != 0 &&
      !InitialParameters.irrev) {
    InitialParameters.irrev = true;
    cout << "Rates Based Mechanism Reduction requires an irreversible scheme - "
            "method set. \n";
  }

  // Intel ODE and odepack have very different tolerance handling... So... We
  // assume tolerances are provided in the common sense way as used by odepack
  // or other code and correct for Intel if Intel is selected as a solver

  if (InitialParameters.Solver_Parameters.SolverType == 0) {
    InitialParameters.Solver_Parameters.atol =
        InitialParameters.Solver_Parameters.atol /
        InitialParameters.Solver_Parameters.rtol;
  }
}
