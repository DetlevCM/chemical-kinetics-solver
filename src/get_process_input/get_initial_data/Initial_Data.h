...
static void Get_Initial_Conditions(
		string,
		vector< Species > ,
		Initial_Data&
);

static void Handle_Initial_Conditions(
...
static void Handle_Solver_Parameters(
		Initial_Data& InitialParameters,
		vector<string> Input
		);

static void Handle_Analysis(
		Initial_Data& InitialParameters,
		vector<string> Input,
		vector<Species> species
		);

static void Handle_Pressure_Vessel(
		Initial_Data& InitialParameters,
		vector<string> Input
		);

static void Handle_PetroOxy(
		Initial_Data& InitialParameters,
		vector<string> Input,
		vector< Species > Species
		);

static mechanism_reduction Handle_Mechanism_Reduction(
		vector<string> Input
		);
