...
vector<Initial_Data::InitSpecies>
Initial_Data::Handle_Species(Initial_Data &InitialParameters,
      ...
      for (j = 0; j < species.size(); j++) {
        if (strcmp(line_content[0].c_str(), species[j].Name.c_str()) == 0) {
          ...
          if (Test_If_Word_Found(Input[i], "ConstantConcentration") ||
            ...
            InitialParameters.ConstantSpecies.push_back(
                j); // set one species constant
