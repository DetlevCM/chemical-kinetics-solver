...
void Initial_Data::Handle_Initial_Conditions(
    Initial_Data &InitialParameters, vector<string> Input,
  ...
  for (i = 0; i < Input.size(); i++) {
    ...
    if (Test_If_Word_Found(Input[i], "TGA")) {
      ...
      // stupid implementation, but works... - maybe I improve it in the
      // future...
      tokens.push_back("!");
      tokens.push_back("//");
      // remove comments
    ...
    if (Test_If_Word_Found(Input[i], "EndTime")) {
      ...
      // store the solver choice - either stores the global setting or the user
      // choice
    ...
    // if not in use, value will be zero - if the user has entered both,
    // activate
    if (InitialParameters.GasPhasePressure != 0 &&
        InitialParameters.GasPhaseVolume != 0) {
      InitialParameters.GasPhase =
          true; // activate Gas Phase Correction for User
    }
