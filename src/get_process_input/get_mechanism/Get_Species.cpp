...
vector<string> Species::Get_Species(string filename) {
    ...
    while (Mechanism_Data.good()) {
      ...
      if (spec_flag && !end_flag) {
        ...
        if (!end_flag) {
          ...
          if (line.find("!") !=
              string::npos) // contains comments, so remove comments
          {
          ...
          if (line.find(" ") != string::npos ||
              line.find("\t") !=
                  string::npos) // contains spaces or tabs, so tokenize
          {
            Tokenise_String_To_String_Append(
                temp_species, line,
                " \t"); // this is really weird, but a " ! " string is required
                        // after a species name
          } else {
            temp_species.push_back(line);
          }
        }
      }
      // Moving species check to end of function avoids "SPECIES" being read in
      // as a name
