...
bool Handle_Mechanism_Input(Filenames filenames,
  ...
  if (DataInputFromFile.is_open()) {
    ...
    WriteReactions("Species_Picked_Reactions.txt", reaction_mechanism.species,
                   reaction_mechanism.reactions);
    Species::ThermodynamicData::Write_Thermodynamic_Data(
   ...
   * initial data file which specifies initial conditions.
   */

  Initial_Data::Get_Initial_Conditions(
      filenames.initial_data, reaction_mechanism.species, InitialParameters);
  ...
  InitialParameters.InitialSpeciesConcentration
      .clear(); // not needed as it should be empty, but doesn't hurt
  ...
  /*
   * We have obtained the ID of the initial species when we read in the input
   * data now it is time to translate this information into initial
   * concentrations
   *
   * We MUST consider whether the user desires Gas Phase Kinetics or Liquid
   * Phase Kinetics - for gas phase kinetics the data is adjusted
   */
  ...
  if (InitialParameters.GasPhase) {
    ...
    for (size_t i = 0; i < Number_Species; i++) {
      ...
      if (temp[i] != 0) {
        cout << reaction_mechanism
                    .species[InitialParameters.InitialLiquidSpecies[i].SpecID]
                    .Name
             << " " << InitialParameters.InitialSpeciesConcentration[i] << "\n";
      }
    }
  ...
  InitialParameters.InitialSpeciesConcentration[Number_Species] =
      InitialParameters.temperature; // Input[1][0];
  cout << "\n";
  ...
  if (InitialParameters.irrev) // contains true of false
  {
    ...
    reaction_mechanism.reactions = Make_Irreversible(
        reaction_mechanism.reactions, reaction_mechanism.species,
        InitialParameters.temperature, 50); // hardcoded to +/- 50K right now
    WriteReactions("irreversible_scheme.txt", reaction_mechanism.species,
                   reaction_mechanism.reactions);
  }
  // End of initial data
   ...
   * to the reaction matrix, so check if the user has supplied a mapping and if
   * yes, handle further. Otherwise ignore the remapping logic
   */

  DataInputFromFile.open("species_mapping.txt");
  if (DataInputFromFile.is_open() &&
      InitialParameters.MechanismReduction.LumpingType > 0) {

    // the species mapping requires an irreversible scheme, check the user has
    // requested that, if not, apply it.
    if (!InitialParameters.irrev) // contains true or false
    {
      printf("Transformation to irreversible scheme required! - This will be "
             "applied first. \n");
      reaction_mechanism.reactions = Make_Irreversible(
          reaction_mechanism.reactions, reaction_mechanism.species,
          InitialParameters.temperature, 50); // hardcoded to +/- 50K for now
      WriteReactions("irreversible_scheme_for_mapping.txt",
                     reaction_mechanism.species, reaction_mechanism.reactions);
    }
    ...
    SpeciesMapping =
        MechanismReduction::Get_Combine_Species_Mapping("species_mapping.txt");
    cout << "Species Mapping Information read in, species will be lumped. \n";

    /*
     * By supplying the mapping, the user has to agreed to try and combine the
     * species. Combining the Species means that the reaction vector is
     * re-mapped
     */

    SpeciesClassMapping = MechanismReduction::Map_Species_Classes(
        SpeciesMapping, reaction_mechanism.species);
    vector<MechanismReduction::ClassNaming> UserDefinedNames =
        MechanismReduction::GetSpeciesClassesNames(SpeciesMapping);
    reaction_mechanism.species = MechanismReduction::RenameSpecies(
        reaction_mechanism.species, UserDefinedNames,
        SpeciesClassMapping); // Update Species Names
    Number_Species_Classes =
        reaction_mechanism.species
            .size(); // need to resize species count a bit later

    reaction_mechanism.species =
        MechanismReduction::Process_Thermodynamics_Species_Classes(
            SpeciesClassMapping,
            reaction_mechanism.species); // create new thermodynamics

    reaction_mechanism.reactions =
        MechanismReduction::Process_Reactions_For_Species_Lumping(
            Number_Species_Classes, SpeciesClassMapping,
            reaction_mechanism.reactions, InitialParameters.temperature,
            InitialParameters.MechanismReduction.UseFastLumping,
            InitialParameters.MechanismReduction
                .LumpingType); // produce new reactions

    Species::ThermodynamicData::Write_Thermodynamic_Data(
        "recombined_thermo.txt", reaction_mechanism.species);
    WriteReactions("recombined_scheme.txt", reaction_mechanism.species,
                   reaction_mechanism.reactions);
    ...
    temp[Number_Species_Classes] =
        InitialParameters
            .InitialSpeciesConcentration[Number_Species]; // reassign initial
                                                          // temperature

    if (InitialParameters.PetroOxy.IsSet) {
      InitialParameters.PetroOxy.GasSpecies =
          SpeciesClassMapping[InitialParameters.PetroOxy.GasSpecies];
    }

    Number_Species = Number_Species_Classes; // Number_Species is accessed later
    ...
    cout << "\nAfter species lumping, the scheme contains the following "
            "counts:\n"
  ...
  if (InitialParameters.PetroOxy.IsSet) {
    ...
    if (InitialParameters.PetroOxy.SampleSize >=
        InitialParameters.PetroOxy.VesselSize) {
      cout << "Pressure vessel is to small, smaller than or equal to the "
              "sample size. \n"
              "Vessel Size is set to 110% of sample size.\n";
      InitialParameters.PetroOxy.VesselSize =
          1.1 * InitialParameters.PetroOxy.SampleSize;
      cout << "Adjusted Pressure Vessel Size: "
           << InitialParameters.PetroOxy.VesselSize << "\n";
    }
    ...
    // Gas Phase Volume
    PetroOxyData.HeadSpaceGas =
        InitialParameters.PetroOxy.VesselSize - PetroOxyData.SampleSize;
    ...
    PetroOxyData.HeadSpaceSolventComponentPressure =
        InitialParameters.PetroOxy.MaxPressure -
    ...
    // atm to Pa - *101325
    PetroOxyData.HenryConstantk =
        101325 / InitialParameters.PetroOxy.GasSolubility;
    // k is now Pa mol / L

    // Mod for limited diffusion
    PetroOxyData.HenryLawDiffusionLimitSet =
        InitialParameters.PetroOxy.HenryLawDiffusionLimitSet;
    PetroOxyData.HenryLawDiffusionLimit =
        InitialParameters.PetroOxy.HenryLawDiffusionLimit;
  }
