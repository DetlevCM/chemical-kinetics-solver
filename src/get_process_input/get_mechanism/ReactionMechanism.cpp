...
void ReactionMechanism::Set_Reactions(const vector< SingleReactionData > & set_reactions)
{
    reactions = set_reactions;
}


void ReactionMechanism::get_mechanism(
		string filename ,
...
)
{
    ...
    vector<Species> species = Species::VectorClass(Species::Get_Species(filename));
	...
	//// NOTE: There is no longer a way to determine how many thermodynamic data entries have been read in
    
    vector< SingleReactionData> reactions = ReactionMechanism::Get_Reactions(filename, species);
...
//// NOTE: above assignment stores data in class ReactionMechanism, implementation is ugly
