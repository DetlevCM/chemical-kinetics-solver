  ...
  if (Mechanism_Data.is_open()) {
    ...
    while (Mechanism_Data.good()) {
      ...
      if (begin_flag && !end_flag) {
        // found = line.find("END"); // need to check for end in loop for 4 line
        // blocks if(Test_If_Word_Found(line,"END") && begin_flag)
      ...
      // in as a name
      if (Test_If_Word_Found(line, "REAC") && line.compare(0, 1, "!") != 0 &&
          begin_flag ==
              0) // only test once... once we found the reactions, its done
      {
        // cout << "REAC found\n";
        begin_flag = 1;
  ...
  for (size_t j = 0; j < Reactions_List.size(); j++) {
    ...
    // DUP
      ...
      if (!line.empty() && line.find_first_not_of("\t ") != string::npos) {
        // Determine if the reaction is irreversible. We assume it is reversible
        // and then correct. irreversible is indicated using => or ->
        bool is_reversible = true;
        if (Test_If_Word_Found(line, "=>") || Test_If_Word_Found(line, "->")) {
          is_reversible = false;
          // assume the user may write "<=>" or "<=>" which he shouldn't...
          if (Test_If_Word_Found(line, "<=>") ||
              Test_If_Word_Found(line, "<->")) {
            is_reversible = true;
          }
        }
        ...
        // SplitLineRight Contains all inputs on the right hand side -> last 3
        // are parameters
        ...
        SplitLineRightSize = SplitLineRight.size() -
                             3; //(last 3 entries are Arrhenius Parameters
        ...
        // Tokenize line, then take last 3 positions - easiest to work on the
        // whole line without comments
        ...
        step = (strtod(SplitLine[SplitLineSize - 3].c_str(),
                       NULL)); // A as read in
        ReactionData[0] =
            Scale_A(step, ReactantData,
                    SchemeUnits[0]); // A in adjusted units for calc.

        ReactionData[1] = (strtod(SplitLine[SplitLineSize - 2].c_str(),
                                  NULL)); // n as read in
          ...
          while (Test_If_Word_Found(Reactions_List[j + 1], "/") &&
                 !Test_If_Word_Found(Reactions_List[j + 1],
                                     "=")) // the initial implementation will
                                           // assume a single line of entries
          {
            // split the string and get the values
            vector<string> PdepTerms =
                Tokenise_String_To_String(Reactions_List[j + 1], " \t/");
            ...
            for (size_t k = 0; k < (PdepTerms.size() / 2);
                 k++) // this WILL fail if the number of elements does not
                      // divide by 2
            {
              for (size_t m = 0; m < Number_Species; m++) {
                if (Test_If_Word_Found(species[m].Name, PdepTerms[2 * k]) &&
                    species[m].Name.size() - PdepTerms[2 * k].size() ==
                        0) // test for same length
                {
            ...
            j = j +
                1; // the loop will end when the next line contains a species
          }
        } else if (temp.ThirdBodyType == 2 &&
                   j + 3 < Reactions_List
                               .size() // we need a minimum of 3 extra lines
        ) {
          // LOW/TROE parameters
          // we can test the next lines for their presence and extract the
          // values
          if (Test_If_Word_Found(Reactions_List[j + 1], "LOW/") &&
              Test_If_Word_Found(Reactions_List[j + 2], "TROE/")) {
            temp.ThBd_LOW =
                Tokenise_String_To_Double(Reactions_List[j + 1], "lowLOW \t/");
            temp.ThBd_TROE = Tokenise_String_To_Double(Reactions_List[j + 2],
                                                       "troeTROE \t/");
