...
vector< SingleReactionData > ReactionMechanism::Get_Reactions(
		string filename,
						...
						for(size_t k=0;k<(PdepTerms.size()/2);k++) // this WILL fail if the number of elements does not divide by 2
						{
							for(size_t m=0;m<Number_Species;m++)
							{
								if(Test_If_Word_Found(species[m].Name,PdepTerms[2*k]) &&
										species[m].Name.size() - PdepTerms[2*k].size() == 0) // test for same length
								{
...
ReactionMechanism::SpeciesWithCoefficient ReactionMechanism::Return_Species_With_Coefficient(
		string InputToSplit,
		const vector< Species > & species
)
{
