      SUBROUTINE SDASKR (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
     *   IDID, RWORK, LRW, IWORK, LIW, RPAR, IPAR, JAC, PSOL,
     *   RT, NRT, JROOT)
C
C***BEGIN PROLOGUE  SDASKR
...
C   040518  Changed adjustment to X2 in Subr. SROOTS.
C   050511  Revised stopping tests in statements 530 - 580; reordered
C           to test for tn at tstop before testing for tn past tout.
C   060712  In SMATD, changed minimum D.Q. increment to 1/EWT(j).
C   071003  In SRCHEK, fixed bug in TEMP2 (HMINR) below 110.
C   110608  In SRCHEK, fixed bug in setting of T1 at 300.
C***CATEGORY NO.  I1A2
...
C
C      CALL SDASKR (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
...
C *Overview
C
C  The SDASKR solver uses the backward differentiation formulas of
...
C  known, in many cases you can have SDASKR solve for them -- see
C  INFO(11). (This and other options are described in detail below.)
C
C  Normally, SDASKR solves the system from T to TOUT.  It is easy to
...
C
C  On each step taken by SDASKR, a sequence of nonlinear algebraic  
...
C  (SDASKR does not allow right preconditioning.)
...
C  given a vector b.
C
C  While integrating the given DAE system, SDASKR also searches for
C  roots of the given constraint functions Ri(T,Y,Y') given by RT.
C  If SDASKR detects a sign change in any Ri(T,Y,Y'), it will return
...
C *Description
C
C------INPUT - WHAT TO DO ON THE FIRST CALL TO SDASKR-------------------
C
C
...
C         statement in your program that calls SDASKR.
...
C         Note: In this respect, SDASKR is not downward-compatible
...
C         Set IRES = -1 if an input value is illegal, and SDASKR
C         will try to solve the problem without getting IRES = -1.
C         If IRES = -2, SDASKR will return control to the calling
C         program with IDID = -11.
C
C         RPAR and IPAR are real and integer parameter arrays which
C         you can use for communication between your calling program
C         and subroutine RES. They are not altered by SDASKR. If you
...
C              SDASKR will perform much better if the matrix of
C              partial derivatives, dG/dY + CJ*dG/dYPRIME (here CJ is
C              a scalar determined by SDASKR), is banded and the code
...
C
C       INFO(11) - SDASKR normally requires the initial T, Y, and
...
C              SDASKR may be able to compute it.
C
C              Denoting the differential variables in Y by Y_d
C              and the algebraic variables by Y_a, SDASKR can solve
...
C              SDASKR by default is downward-compatible with SDASSL,
...
C          ****   Do you want SDASKR to use standard direct methods
...
C              SDASKR uses scalars MAXL, KMP, NRMAX, and EPLI for the
...
C               The usage of SDASKR is simpler if PSOL can carry out
...
C
C       INFO(17) - used when INFO(11) > 0 (SDASKR is to do an 
C              initial condition calculation).
C              SDASKR uses several heuristic control quantities in the
...
C             SDASKR and the RES subroutine (and the JAC and PSOL
C             subroutines).  They are not altered by SDASKR.
...
C           forms).  They are not altered by SDASKR.
...
C         your program that calls SDASKR.
C
...
C         SDASKR will find roots of the Ri of odd multiplicity
...
C         due to roundoff and integration error, SDASKR may return
...
C         the input to SDASKR should nevertheless allow
C         integration to a point slightly past that root, so
C         that SDASKR can locate the root by interpolation.
C
...
C
C  SDASKR uses a weighted root-mean-square norm to measure the 
...
C
C  A SDAWTS routine has been included with SDASKR which sets the
...
C  norm used in SDASKR (weighted root-mean-square) is not subject
C  to replacement by the user.  In this respect, SDASKR is not
C  downward-compatible with the original SDASSL solver (in which
C  the norm routine was optionally user-replaceable).
C
C
C------OUTPUT - AFTER ANY RETURN FROM SDASKR----------------------------
C
...
C
C           IDID = -6 -- SDASKR had repeated error test failures on the
...
C
C           IDID =-12 -- SDASKR failed to compute the initial Y, YPRIME.
C
...
C                  In this situation, the next call to SDASKR is 
...
C                  attempted step in SDASKR.  A singularity in the
...
C                  attempted step in SDASKR.  An inaccurate or ill-
...
C                  may be redundant.  SDASKR cannot solve the problem
C                  as stated.  It is possible that the redundant
C                  equations could be removed, and then SDASKR could
C                  solve the problem.  It is also possible that a
C                  solution to your problem either does not exist
C                  or is not unique.
C
C     IDID = -9, SDASKR had multiple convergence test failures, preceded
...
C
C     IDID = -10, SDASKR had multiple convergence test failures
...
C
C     IDID = -12, SDASKR failed to compute the initial Y and YPRIME
...
C***ROUTINES CALLED
C
C   The following are all the subordinate routines used by SDASKR.
C
C   SRCHEK does preliminary checking for roots, and serves as an
C          interface between Subroutine SDASKR and Subroutine SROOTS.
...
C   R1MACH provides the unit roundoff of the computer.
...
C          routines.
C
C The routines called directly by SDASKR are:
C   SCNST0, SDAWTS, SINVWT, R1MACH, SDWNRM, SDASIC, SDATRP, SDSTP,
C   SRCHEK, XERRWV
C
C***END PROLOGUE SDASKR
C
C
      ...
      DIMENSION RPAR(*),IPAR(*)
      CHARACTER MSG*80
      EXTERNAL  RES, JAC, PSOL, RT, SDASID, SDASIK, SNEDD, SNEDK
C
...
C
C
C***FIRST EXECUTABLE STATEMENT  SDASKR
C
C
...
C
      ...
      CALL SRCHEK(1,RT,NRT,NEQ,T,TOUT,Y,YPRIME,RWORK(LPHI),
...
C
      CALL SRCHEK(2,RT,NRT,NEQ,TN,TOUT,Y,YPRIME,RWORK(LPHI),
     *   RWORK(LPSI),IWORK(LKOLD),RWORK(LR0),RWORK(LR1),
     *   RWORK(LRX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *   RWORK,IWORK,RPAR,IPAR)
      IF (IRT .LT. 0) GO TO 731
...
C     Check for a zero of R near TN.
C
      CALL SRCHEK(3,RT,NRT,NEQ,TN,TOUT,Y,YPRIME,RWORK(LPHI),
     *   RWORK(LPSI),IWORK(LKOLD),RWORK(LR0),RWORK(LR1),
     *   RWORK(LRX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *   RWORK,IWORK,RPAR,IPAR)
...
C     Stopping tests for the TSTOP case, interval-output mode. ---------
      IF (ABS(TN-TSTOP) .LE. 100.0E0*UROUND*(ABS(TN)+ABS(H))) THEN
         CALL SDATRP(TN,TSTOP,Y,YPRIME,NEQ,IWORK(LKOLD),
     *               RWORK(LPHI),RWORK(LPSI))
         T = TSTOP
         IDID = 2
...
C
C-----------------------------------------------------------------------
C     All successful returns from SDASKR are made from this block.
C-----------------------------------------------------------------------
C
590   CONTINUE
      RWORK(LTN)=TN
...
620   MSG = 'DASKR--  AT T (=R1) TOO MUCH ACCURACY REQUESTED'
      ...
      CALL XERRWV(MSG,43,622,0,0,0,0,1,R,0.0E0)
      GO TO 700
C
...
722   MSG = 'DASKR--  NRMAX (=I1) ILLEGAL. .LT. 0'
      ...
      GO TO 750
723   MSG = 'DASKR--  EPLI (=R1) ILLEGAL. EITHER .LE. 0.E0 OR .GE. 1.E0'
...
730   MSG = 'DASKR--  NRT (=I1) .LT. 0'
      CALL XERRWV(MSG,25,30,1,1,NRT,0,0,0.0E0,0.0E0)
      GO TO 750
731   MSG = 'DASKR--  R IS ILL-DEFINED.  ZERO VALUES WERE FOUND AT TWO'
      CALL XERRWV(MSG,57,31,1,0,0,0,0,0.0E0,0.0E0)
      MSG = '         VERY CLOSE T VALUES, AT T = R1'
      CALL XERRWV(MSG,39,31,1,0,0,0,1,RWORK(LT0),0.0E0)
C
...
770   MSG = 'DASKR--  RUN TERMINATED. APPARENT INFINITE LOOP'
      ...
      RETURN
C
C------END OF SUBROUTINE SDASKR-----------------------------------------
      END
      SUBROUTINE SRCHEK (JOB, RT, NRT, NEQ, TN, TOUT, Y, YP, PHI, PSI,
     *   KOLD, R0, R1, RX, JROOT, IRT, UROUND, INFO3, RWORK, IWORK,
     *   RPAR, IPAR)
C
C***BEGIN PROLOGUE  SRCHEK
C***REFER TO SDASKR
C***ROUTINES CALLED  SDATRP, SROOTS, SCOPY, RT
C***REVISION HISTORY  (YYMMDD)
C   020815  DATE WRITTEN   
...
C Pointers into RWORK:
      ...
      REAL TN, TOUT, Y, YP, PHI, PSI, R0, R1, RX, UROUND,
     *  RWORK, RPAR
      DIMENSION Y(*), YP(*), PHI(NEQ,*), PSI(*),
     *          R0(*), R1(*), RX(*), JROOT(*), RWORK(*), IWORK(*)
      INTEGER I, JFLAG
      REAL H
      REAL HMINR, T1, TEMP1, TEMP2, X, ZERO
      LOGICAL ZROOT
      DATA ZERO/0.0E0/
C-----------------------------------------------------------------------
...
C     
      ...
      DO 10 I = 1,NRT
 10     JROOT(I) = 0
      HMINR = (ABS(TN) + ABS(H))*UROUND*100.0E0
C
...
C Set T1 to TN or TOUT, whichever comes first, and get R at T1. --------
 ...
 330  CALL SDATRP (TN, T1, Y, YP, NEQ, KOLD, PHI, PSI)
      CALL RT (NEQ, T1, Y, YP, NRT, R1, RPAR, IPAR)
      IWORK(LNRTE) = IWORK(LNRTE) + 1
C Call SROOTS to search for root in interval from T0 to T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL SROOTS (NRT, HMINR, JFLAG, RWORK(LT0),T1, R0,R1,RX, X, JROOT)
...
C---------------------- END OF SUBROUTINE SRCHEK -----------------------
      END
      SUBROUTINE SROOTS (NRT, HMIN, JFLAG, X0, X1, R0, R1, RX, X, JROOT)
C
...
C          JFLAG = 1 means SROOTS needs a value of R(x).  Set RX = R(X)
...
C-----------------------------------------------------------------------
      ...
      REAL ALPHA, T2, TMAX, X2, FRACINT, FRACSUB,
     1                 ZERO, TENTH, HALF, FIVE
      LOGICAL ZROOT, SGNCHG, XROOT
      SAVE ALPHA, X2, IMAX, LAST
      DATA ZERO/0.0E0/, TENTH/0.1E0/, HALF/0.5E0/, FIVE/5.0E0/
C
...
C At this point, R0(i) has been checked and cannot be zero. ------------
 110    IF (SIGN(1.0E0,R0(I)) .EQ. SIGN(1.0E0,R1(I))) GO TO 120
...
C Neither R0(i) nor RX(i) can be zero at this point. -------------------
 210    IF (SIGN(1.0E0,R0(I)) .EQ. SIGN(1.0E0,RX(I))) GO TO 220
 ...
 300  JFLAG = 2
      ...
      DO 320 I = 1,NRT
        ...
        IF (ABS(R1(I)) .EQ. ZERO) THEN
          ...
          GO TO 320
          ENDIF
        IF (SIGN(1.0E0,R0(I)) .NE. SIGN(1.0E0,R1(I)))
     1     JROOT(I) = SIGN(1.0E0,R1(I) - R0(I))
 320    CONTINUE
      RETURN
C
...
C Zero value at X1 and no sign change in (X0,X1).  Return JFLAG = 3. ---
      ...
      DO 410 I = 1,NRT
        JROOT(I) = 0
        IF (ABS(R1(I)) .EQ. ZERO) JROOT(I) = -SIGN(1.0E0,R0(I))
 410  CONTINUE
      JFLAG = 3
      RETURN
C
C No sign changes in this interval.  Set X = X1, return JFLAG = 4. -----
 420  CALL SCOPY (NRT, R1, 1, RX, 1)
      X = X1
      JFLAG = 4
      RETURN
C----------------------- END OF SUBROUTINE SROOTS ----------------------
      END
      SUBROUTINE SDASIC (X, Y, YPRIME, NEQ, ICOPT, ID, RES, JAC, PSOL,
...
C------END OF FUNCTION SDWNRM-------------------------------------------
      END
      SUBROUTINE SDASID(X,Y,YPRIME,NEQ,ICOPT,ID,RES,JACD,PDUM,H,TSCALE,
...
C
      ...
      FNRM = DELNRM
      IF (TSCALE .GT. 0.0E0) FNRM = FNRM*TSCALE*ABS(CJ)
...
C
      OLDFNM = FNRM
C
      CALL SLINSD (NEQ, Y, X, YPRIME, CJ, TSCALE, DELTA, DELNRM, WT,
...
C------END OF SUBROUTINE SNSID------------------------------------------
      END
      SUBROUTINE SLINSD (NEQ, Y, T, YPRIME, CJ, TSCALE, P, PNRM, WT,
...
C
C***ROUTINES CALLED
C   SFNRMD, SYYPNW, SCNSTR, SCOPY, XERRWV
C
...
C
      ...
      IF (KPRIN .GE. 2) THEN
        ...
        CALL XERRWV(MSG, 38, 901, 0, 0, 0, 0, 1, PNRM, 0.0E0)
        ENDIF
      TAU = PNRM
      RL = ONE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 100  CONTINUE
      CALL SYYPNW (NEQ,Y,YPRIME,CJ,RL,P,ICOPT,ID,YNEW,YPNEW)
      CALL SFNRMD (NEQ, YNEW, T, YPNEW, R, CJ, TSCALE, WT, RES, IRES,
...
C----------------------- END OF SUBROUTINE SLINSD ----------------------
      END
      SUBROUTINE SFNRMD (NEQ, Y, T, YPRIME, R, CJ, TSCALE, WT,
...
C-----------------------------------------------------------------------
      ...
      IF (TSCALE .GT. 0.0E0) FNORM = FNORM*TSCALE*ABS(CJ)
C
      RETURN
...
200   IRES=0
      ...
      SQUR = SQRT(UROUND)
      DO 210 I=1,NEQ
         DEL=MAX(SQUR*MAX(ABS(Y(I)),ABS(H*YPRIME(I))), 1.0E0/EWT(I))
...
500   MBAND=IWM(LML)+IWM(LMU)+1
      ...
      DO 540 J=1,MBA
        DO 510 N=J,NEQ,MBAND
          ...
          DEL=MAX(SQUR*MAX(ABS(Y(N)),ABS(H*YPRIME(N))), 1.0E0/EWT(N))
...
C------END OF SUBROUTINE SSLVD------------------------------------------
      END
      SUBROUTINE SDASIK(X,Y,YPRIME,NEQ,ICOPT,ID,RES,JACK,PSOL,H,TSCALE,
...
C
      CALL SFNRMK (NEQ, Y, X, YPRIME, SAVR, R, CJ, TSCALE, WT,
...
C
      OLDFNM = FNRM
C
      CALL SLINSK (NEQ, Y, X, YPRIME, SAVR, CJ, TSCALE, DELTA, DELNRM,
...
C     The maximum number of iterations was done.  Set IERNEW and return.
C
 380  IF (RATE .LE. RATEMX .OR. FNRM .LE. 0.1E0*FNRM0) THEN
         IERNEW = 1
      ELSE
         IERNEW = 2
      ENDIF
...
C----------------------- END OF SUBROUTINE SNSIK------------------------
      END
      SUBROUTINE SLINSK (NEQ, Y, T, YPRIME, SAVR, CJ, TSCALE, P, PNRM,
     *   WT, SQRTN, RSQRTN, LSOFF, STPTOL, IRET, RES, IRES, PSOL,
...
C
C***ROUTINES CALLED
C   SFNRMK, SYYPNW, SCNSTR, SCOPY, XERRWV
C
...
C
      IF (KPRIN .GE. 2) THEN
        ...
        CALL XERRWV(MSG, 38, 921, 0, 0, 0, 0, 1, PNRM, 0.0E0)
        ENDIF
      TAU = PNRM
      RL = ONE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 100  CONTINUE
      CALL SYYPNW (NEQ,Y,YPRIME,CJ,RL,P,ICOPT,ID,YNEW,YPNEW)
      CALL SFNRMK (NEQ, YNEW, T, YPNEW, SAVR, R, CJ, TSCALE, WT,
...
C----------------------- END OF SUBROUTINE SLINSK ----------------------
      END
      SUBROUTINE SFNRMK (NEQ, Y, T, YPRIME, SAVR, R, CJ, TSCALE, WT,
...
C***DESCRIPTION
C
C This is similar to the LINPACK routine SGESL except that
