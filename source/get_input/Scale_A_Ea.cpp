...
double Scale_A(double A_read_in, vector<double> ReactantData, int scaling_type)
{
	if(scaling_type == 0) // A is molecules / cm^3
	{
		...
		return A_read_in/1000; // and convert to molecules per liter (dm^3)
	}
	else if(scaling_type == 1) // A is in moles / cm^3
	{
		...
		return A_read_in/1000; // adjust to moles per liter (dm^3)
	}
...
double Scale_Ea(double Ea_read_in, int scaling_type)
{
	...
	{
		return Ea_read_in/8.3144621e-3; // sure input gets converted
	}

	else if(scaling_type == 4) // Ea is in J/mol
	{
		return Ea_read_in/8.3144621; // sure input gets converted
	}
